...
void *read_thread_fun(void* arg)
{
    ...
    int64_t t2 = getRealTimeUs() - t1;
    LOGE( "all_count: %lld t2 = %lld all_time: %-20lld  ecount: %-lld\n", count, t2,all_time, ecount);
    memset(buffer, 0, LOG_MAX_SIZE);
    sprintf(buffer, "\nall_count: %lld t2 = %lld all_time: %-20lld  ecount: %-lld\n", count, t2, all_time, ecount);
    write(_fd, buffer, strlen(buffer));
