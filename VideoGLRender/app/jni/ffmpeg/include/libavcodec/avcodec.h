...
#ifdef LOOP_FILTER_OPTIMIZE  // loop filter optimize by wjb
enum loop_filter_opt
{
    loop_filter_opt_strong,   //loop filter function is closed,so the influence is strong
    loop_filter_opt_middle,   //loop filter optmization includes amplitude limiting¡¢chroma optimization
                              //and removing a point,the influence is middle
    loop_filter_opt_weak,     //loop filter optmization includes amplitude limiting¡¢chroma optimization
                              //so the influence is the weakest
    loop_filter_opt_closed,   //loop filter function is opened completely, so the optimization is closed
} ;
#endif
...
#ifdef LOOP_FILTER_OPTIMIZE  // loop filter optimize by wjb
    enum loop_filter_opt loop_filter_opt_level_flag;
#endif
} AVCodecContext;
